...
prvo je potrebno inicijalizovati \emph{Elm} projekat (\texttt{\textbf{elm init}}) i u \texttt{src} folderu napraviti 
...
U korenu projekta nalazi se osnovna \emph{Main.elm} datoteka u kojoj se nalazi funkcija \texttt{main} sa definicijom \emph{Elm} aplikacije.
...
putanji i stranici koja se prikazuje. Takođe, model sadrži podatke o aktivnostima čiji sadržaj zavisi od trenutnog korisnika, \emph{Url} veb interfejsa,
...
predstavlja funkcija \texttt{update}, čiji je povratni tip, kao i kod \texttt{init} funkcije, proširen sa \texttt{\textbf{Cmd} msg}.
