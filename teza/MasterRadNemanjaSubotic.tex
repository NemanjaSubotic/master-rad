...
Cilj ovog rada je implementacija portala MSNR.
...
razvojnim okvirima klijentskog programiranja. Ideja je da se u memoriji čuva "virtuelna" reprezentacija
...
\textbf{Haskell}\cite{haskell} je čist funkcionalni programski jezik, naziv je dobio po matematičaru i
...
U ovom poglavlju predstavljene su osnove programskog jezika Elm --- osnovni tipovi i strukture podataka,
...
Karakteri se navode između dva apostrofa (\texttt{'a', '0', '\textbackslash t'}...), a
...
Tip \textbf{Float} služi za predstavljanje brojeva u pokretnom zarezu po standardu
...
dok se simbol \texttt{\textbf{->}} koristi umesto znaka pridruživanja (primer koda \ref{listing:lambda}).
\begin{listing}[h]
\begin{minted}{haskell}
> broj3 = 3
3 : number
...
dovodi do lakšeg uočavanja i otklanjanja grešaka. Pored toga, anotacije predstavljaju veoma
...
Osnovne strukture podataka programskog jezika \emph{Elm} čine liste, torke i slogovi. 
\subsubsection{Liste}
...
sa listama nalaze se unutar modula \texttt{List}, među kojima su i tri veoma važne funkcije višeg reda
...
Funkcija \textbf{\texttt{map}} prihvata funkciju i listu i primenjuje datu funkciju na svaki element
...
U zavisnosti od smera "savijanja" --- da li se ide sa početka ili kraja liste koriste se leva (\textbf{\texttt{foldl}}),
odnosno desna (\textbf{\texttt{foldr}}) funkcija. Funkcije (\textbf{\texttt{fold}}) prihvataju binarnu funkciju,
početnu vrednost akumulatora i listu i prilikom prolaska kroz elemente liste primenjuje se data funkcija
...
\texttt{Tuple}, dok se za rad sa torkama od tri elementa koristi poklapanje obrazaca
...
: a -> b -> ( a, b )} mogu biti istog tipa. Prilikom anotacije tipova mogu se koristiti i
...
kao i \emph{unijskim} tipovima, jer mogu predstavljati uniju više varijanti definisanog tipa. Definišu se ključnom reči 
...
i definicija binarnog stabla kao primer rekurzivne strukture.
\begin{listing}[h]
\begin{minted}{haskell}
...
Za razliku od izraza \texttt{\textbf{if}}, izraz \texttt{\textbf{case}} omogućava grananje na
osnovu šireg raspona vrednosti, umesto samo sa \emph{tačno} ili \emph{netačno}. Takođe, pruža mogućnost navođenja više grana.
Primer upotrebe izraza \texttt{\textbf{case}} prikazan je u primeru koda \ref{listing:case}. Vrednost navedena nakon ključne reči
...
može se koristiti simbol \texttt{\textbf{\textunderscore}}. Izraz \texttt{\textbf{case}}
...
vrednostima se može pristupiti putem identifikatora definisanim u obrascu. Pored spomenutih
\texttt{\textbf{case}} izraza, poklapanje obrazaca može se koristiti u vidu 
...
i ostale slične vrednosti prisutne u drugim programskim jezicima.
...
\begin{enumerate}
  ...
  \item Pogled (eng. \emph{View}) - transformacija stanja u \emph{HTML}
...
u arhitekturi \emph{Elm} celokupno stanje aplikacije se nalazi na jednom mestu, tj. modelu, a
...
funkcije koje nam omogućavaju kreiranje različitih tipova programa, među kojima je i
...
je u primeru koda \ref{listing:elmProgram}.
\begin{listing}[h!]
\begin{minted}{haskell}
...
\textbf{\texttt{main}} se može definisati funkcijama iz modula \texttt{\textbf{Html}},
...
Ukoliko dođe do odgovarajuće akcije korisnika, u ovom slučaju klikom na dugme, okruženje \emph{Elm}
...
(eng. \emph{data ingestion}) i u domenima za obradu multimedije.
\end{displayquote}
...
sistemi. Provera uspešne instalacije može se izvršiti pokretanjem komande
...
je koristiti i razmake, tada se sadržaj posle dvotačke mora navesti unutar navodnika.
...
"1+1=2"
\end{minted}
\caption{Primeri niski u \emph{Elixir}-u}
...
"abc"
\end{minted}
\caption{Predstavljane niski kao niz bajtova}
...
već se za to mogu koristiti isključivo anonimne funkcije. Ovaj nedostatak imenovanih funkcija se može prevazići upotrebom
...
funkciji sa desne strane, što je različito ponašanje koje isti operator ima u \emph{Elm}-u, gde se
...
\subsubsection{Liste}
Kao i u \emph{Elm}-u, lista kao tip predstavlja jednostruko povezanu listu, čiji se elementi navode
...
\texttt{\textbf{List}}, kao i funkcije iz modula \texttt{\textbf{Enum}} o kom će biti više reči kasnije. Primeri upotrebe nekih od navedenih funkcija prikazani su u primeru koda \ref{listing:elixirList}.
\begin{listing}[h]
\begin{minted}{elixir}
...
\texttt{\textbf{[\smallskip]}} za pristup po određenoj ključnoj reči. Prikaz nekih od funkcija i operatora dat je u primeru koda \ref{listing:keywordList}.
\begin{listing}[!h]
\begin{minted}{elixir}
...
uglastih zagrada. U suprotnom obavezno je njihovo korišćenje, pri čemu se nenavedenim ključevima
...
poklapanje obrazaca koristi isključivo unutar izraza \textbf{\texttt{case}} i
...
ponovno vezivanje možemo koristiti \emph{pin} operator \textbf{\texttt{ \^{}}}.
\begin{listing}[!h]
\begin{minted}{elixir}
iex> x = 1
1
...
ali i za preopterećivanje funkcija iste arnosti korišćenjem različitih šablona.
...
\subsection{Kontrola toka}
\emph{Elixir} nema izraze ili naredbe za kontrolu toka i prethodno je prikazano kako
...
Slično \emph{Elm}-u, \emph{Elixir} umesto petlji koristi funkcije višeg reda i rekurziju, s tim
...
\end{listing}
Ipak, u nekim vrlo retkim slučajevima kada neke biblioteke ili delovi koda nisu implementirani
u duhu funkcionalnog programiranja, moguća je obrada grešaka slična većini drugih programskih
jezika --- korišćenjem \texttt{try}, \texttt{catch} i \texttt{rescue} mehanizma o kom se može
...
U prvom delu ove glave predstavljen je deo razvojnog okvira \emph{Phoenix} na primeru
portala MSNR, nakon čega je opisana implementacija osnovnih delova samog portala.
...
sa samim jezikom. Jedan od autora razvojnog okvira je i Žozé Valim (autor jeziku \emph{Elixir}).
...
\emph{Phoenix} je okvir za razvoj svih tipova veb aplikacija, omogućava iscrtavanje \emph{HTML}-a na serverskoj strani i ima odličnu podršku
...
Takođe, portal ne podržava lokalizaciju, stoga nema potrebe za uključivanje modula \emph{gettext}.
...
Kontekst \emph{MsnrApi} se odnosi na \emph{Model} u smislu arhitekture \emph{MVC --- Model-View-Controller} i u njemu će se definisati svi entiteti i
...
u tokove (eng. \emph{pipeline}). Unutar modula \emph{Router}, na osnovu putanje iz zahteva definišu se opsezi
...
\emph{Phoenix} projekta dodaju se potrebni \emph{Ecto} moduli i konfiguracija. Ukoliko nije potrebno
...
proces prihvatanja eksternih podataka, validiranja, konvertovanja i proveravanja dodatnih uslova pre nego što se upišu u bazu. Osnovni moduli biblioteke su:
\begin{itemize}
...
opcija za postavljanje podrazumevane vrednosti kolone. Funkcija \texttt{\textbf{timestamps}} dodaje još dva polja,
...
koje predstavlja primarni ključ tabele. U slučaju kada je potrebno drugačije definisati primarni ključ koristi
...
i drugi sa dve funkcije \texttt{\textbf{up}} i \texttt{\textbf{down}}, kojima se difinišu akcije koje se pozivaju prilikom izvršavanja i
...
\end{minted}
\caption{Definicija modula \emph{MsnrApi.ActivityTypes}}
...
\end{listing}
Razvojni okvir \emph{Phoenix} pruža veliki broj komandi za generisanje, kao što su \texttt{mix phx.gen.context}, 
...
\end{listing}
Struktura \emph{Ecto.Multi} se koristi za grupisanje više operacija nad repozitorijumom, pri čemu se svakoj
...
\item \emph{refresh{\textunderscore}token} --- Koristi se za obnavljanje \emph{acceess{\textunderscore}token}, ima duže vreme trajanja (nekoliko dana) i prosleđuje se u \emph{HttpOnly} i \emph{Secure} kolačiću.  
\end{itemize}
...
Osnovni tipovi aktivnosti (prijava grupe, predaja CV-a, rada i recenzije) biće inicijalno ubačene u
...
izvršava se dodeljena aktivnost. Priloženi dokumenti se čuvaju na serveru u direktorijumu podešenom u konfiguraciji.
...
su navedene sve putanje veb interfejsa koje se koriste u aplikaciji.
...
modula \emph{AssignmentContent}. U primeru koda \ref{listing:student-model} predstavljen je osnovni model studentske stranice u kom se
...
\chapter{Zaključak}
% ------------------------------------------------------------------------------
U ovom radu predstavljen je razvoj studentskog portala upotrebom funkcionalnih
...
U toku rada nije predstavljena integracija sa \emph{JavaScript}-om, koja je moguća, ali
podrazumeva uvođenje dosta šablonskog koda. Stoga, \emph{Elm} ne predstavlja pravi izbor
za razvoj aplikacija koje se zasnivaju na velikom broju biblioteka napisanih u programskom
