...
\kljucnereci{programski jezici, funkcionalno programiranje, programski jezik \emph{Elm}, programski jezik \emph{Elixir}, razvojno okruženje \emph{Phoenix}}
...
Takođe, portal neće podržavati lokalizaciju, stoga nema potreba za uključivanje modula \emph{gettext}.
...
funkcije iz modula \texttt{Main} i tom prilikom se vrši prosleđivanje putanje ka veb interfjesu, tj. serverskom delu portala,
preko oznaka.
\begin{listing}[h]
...
Za komuniciranje sa MSNR veb interfejsom kreiran je modul \emph{Api} koji otkriva funkcije \texttt{get}, \texttt{post},
...
token. Funkcijom \texttt{Time.every} se periodično, na određeni broj milisekundi uzima trenutno vreme i prosleđuje se
...
\begin{listing}[h]
  ...
  \end{listing}
Unutar funkcjie \texttt{update} glavnog modula se na osnovu poruke poziva funkcija \texttt{update}
...
ipak zgodno je iskoristiti gotove stilizovane komponente (npr. modale, tabove i slično) kako ne bismo
...
primer njene upotrebe unutar modula \emph{Main} prikazana je u listingu \ref{listing:elm-toUnstyled}.
...
\emph{ProgessorPage}, slično obradi svih stranica unutar modula \emph{Main}. 
