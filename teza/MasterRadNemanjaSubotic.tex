...
konkatenaciju niski i listi. Listing \ref{listing:operatori} prikazuje primere upotrebe 
navedenih operatora.
...
Da bi se modul iz listinga \ref{listing:modul} koristio u interpretatoru (\texttt{\textbf{elm repl}}),
...
listingu \ref{listing:listFnTypes}, a njihova upotreba u listingu \ref{listing:liste}.
\begin{listing}[h]
\begin{minted}{haskell}
> List.map
...
tip koji se definiše se može navoditi unutar svoje definicije. U listingu \ref{listing:algebraic}
...
Primer upotrebe izaraza \texttt{\textbf{case}} prikazan je u listingu \ref{listing:case}. Vrednost navedena nakon ključne reči
...
\texttt{:''Elixir.<naziv aliasa>''}. Upotreba atoma prikazana je u listingu \ref{listing:atomi}.
...
\emph{Elixir} pruža funkcije za proveru tipa podataka, među kojima su \texttt{\textbf{is{\textunderscore}atom}} i \texttt{\textbf{is{\textunderscore}boolean}} i čija upotreba je predstavljena u listingu \ref{listing:boolAtomi}.
\begin{listing}[!h]
\begin{minted}{elixir}
iex> is_atom(true)
true
...
niski, što se može videti u listingu \ref{listing:elixirString}.
\begin{listing}[!h]
\begin{minted}{elixir}
iex> "Ćao!"
"Ćao!"
...
\end{listing}
U listingu \ref{listing:elixirBitstring} vidimo da ukoliko sadržaj niske bitova čine samo ASCII
...
prilikom definisanja funkcija. Primer preopterećivanja funkcije upotrebom čuvara dat je u listingu
\begin{listing}[!h]
\begin{minted}{elixir}
defmodule Math do
...
\texttt{\textbf{List}}, kao i funkcije iz modula \texttt{\textbf{Enum}} o kom će više rečeno kasnije. Primeri upotrebe nekih od navedenih funkcija prikazani su u listingu \ref{listing:elixirList}.
\begin{listing}[h]
\begin{minted}{elixir}
...
\texttt{\textbf{[\smallskip]}} za prisup po određenoj ključnoj reči. Prikaz nekih od funkcija i operatora dat je u listingu \ref{listing:keywordList}.
\begin{listing}[!h]
\begin{minted}{elixir}
...
a moguće je i ažuriranje elemenata. Pored funkcija iz modula, dostupna je i sintaksa ažuriranja slična sintaksi ažuriranja slogova u \emph{Elm}-u i prikazana je u listingu \ref{listing:elixirMap2}.
\begin{listing}[!h]
\begin{minted}{elixir}
...
Upotreba  nekih od funkcija iz modula \texttt{Enum} nad listama, opsezima i mapama se može videti u listingu \ref{listing:elixirEnum}. Funkcije iz modula \texttt{Enum}
...
Primer preopterećivanja funkcije upotrebom poklapanja obrazaca dat je u listingu
...
u listingu \ref{listing:elixirIf}. 
\begin{figure}[!h]
\begin{minipage}{0.5\textwidth} 
  \raggedleft 
  \begin{minted}{elixir}
...
\texttt{nil}. Postoje i skraćene, jednolinijske verzije sintaksi i prikazane su u listingu
...
Primeri nekoliko implementacija protokola iz listinga \ref{listing:elixirDefProtocol} može se videti u listingu \ref{listing:elixirImplProtocol}.
\begin{listing}[t]
\begin{minted}{elixir}
...
\emph{Hex} \cite{hex} --- menadžer paketa za ekosistem \emph{Erlang}. Instalacija potrebnih alatki prikazana je u listingu \ref{listing:installPhxNew}.
