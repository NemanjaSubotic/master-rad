...
Elm sa sobom donosi niz alata (tabela \ref{table:elmTools}) i Elm okruženje (eng. \emph{Elm 
...
\begin{itemize}
  ...
  \item Odabir tema za tekucu godinu
...
zahteve za registracije, dodaje aktivnosti i teme za tekuću godinu, i ocenjuje izvršene studentske aktivnosti. Pored studentskog i
profesorskog dela aplikacije, svi posetioci portala imaće pristup biblioteci studentskih radova nastalih tokom pohađanja kursa.
...
Na klijentskoj strani se nalazi korisnički interfejs koji je implementiran u Elm-u kao jednostranična aplikacija (eng. \emph{Single Page Application --- SPA}).
Središnji sloj predstavlja aplikacioni veb interfejs - (eng. \emph{Web Appilicatoin Programming Interface --- API})
koji je pomoću Phoenix razvojnog okruženja implementiran u stilu REST arhitekture.
...
\subsection{Korisnik}
Tabela \emph{users} predviđena je za upravljanje korisničkim nalozima. Mogu postojati dva tipa, odnosno role, korisnika --- student i profesor. 
...
drugi u tabelu koja predstavlja relaciju studenta i semestra --- \emph{students{\textunderscore}semestars}. Ova tabela, pored referenci ka studenet i semestru, ima i referencu ka grupi tako da 
...
Navedene aktivnosti na početku poglavlja su zapravo tipovi aktivnosti koji su predstavljeni tabelom \emph{activity{\textunderscore}types}. Dok aktivnost predstavlja određeni tip aktivnosti
u jednom semestru. Tip aktivnosti sadrži naziv i opis aktivnosti, jedinstven k\^{o}d, oznake da li ima prijavu i da li je grupna aktivnost, i sam sadržaj aktivnosti. Aktivnost ima referencu ka semestru, period u kom se aktivnost
može izvršiti, broj poena, referncu na tip aktivnosti, kao i oznaku da li je samo prijava.
...
Upravo se pomoću njega instalira \emph{Phoenix} radno okruženje i alat za generisanje \emph{Phoenix} projekata, ali prethodno je potrebno instalirati
\emph{Hex} --- menažer paketa za erlang ekosistem. Instalacija potrebnih alata prikazana je u listingu \ref{listing:installPhxNew}.
...
za komunikaciju u realnom vremenu. Poseduje i svojstven LiveView koncept koji pruža bogato korisničko istustvo u realnom vremenu sa
renderovanjem na serverskoj strani. Za potrbe implementacije MSNR portala kreiraće se jednostavan veb API i prethodno navedene mogućnosti nisu potrebne.
Komanda za kreiranje projketa:
...
Nakon što se komanda uspešno izvrši, dobija se struktura projekta prikazana na slici \ref{fig:msnr-dir}.
...
\emph{Phoenix} dodaje i konfiugracije vezana za okruženje u kom se aplikacija izvršava. 
...
\emph{Phoenix} projetki se organizuju po konetekstima (eng. \emph{context}). Kontekst predstavlja modul koji grupiše funkcije sa
...
Kontekst \emph{MsnrApi} se u odnosi na \emph{Model} u smislu \emph{MVC --- Model-View-Controller} arhitekture. Sadržaće definicje entiteta i
...
Osnovne komponente \emph{Phoenix} radnog okruženja, kao što su \emph{Endpoint}, ruter i kontrolori, su interno utikači (eng. \emph{plug}).
...
čime se završava obrada zahteva u okviru \emph{Phoenix} okruženja. 
% Celokupna obrada zahteva u okviru \emph{Phoenix} okruženja prikazana je u listingu 
...
funkciji \texttt{all} se prosleđuje samo naziv tabela, tako da funkcija vraća sve redove. Definisanje upita biće objašnjeno u poglavlju.
\begin{listing}[!h]
  \begin{minted}[fontsize=\footnotesize]{elixir}
...
proveravaju obavezna polja i jedistvenost naziva i koda.  Izvršavanje funkcija jednom za drugom, menja se stanje
...
\section{Prijavljivanje korisnkia}


\section{Autorizacija korisnika}


\section{Ubacivanje i dodela aktivnosti}


\section{Izvrsavanje aktivnosti}
