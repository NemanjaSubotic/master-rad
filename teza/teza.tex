...
\apstr{%
Funkcijonalni programski jezici \emph{Elm} i \emph{Elixir} pripadaju novoj generaciji
...
Cilj rada je prikaz najbitnijih karakteristika i koncepata navedenih porgramskih jezika, kao i
...
\kljucnereci{programski jezici, funkcijonalno programiranje, \emph{Elm}, \emph{Elixir}, \emph{Phoenix}}
...
napreduje razvoj podrške funkcijonalnim konceptima, kao i razvoj novih funkcionalnih
...
poslužila kao inspiracija mnogim \emph{JavaScript} biblotekama. Detaljni opis
jezika i navedenih karakteristika dat je u drugom poglavlju.

U trećem poglavlju predstavljen je programski jezik \emph{Elixir} koji nastaje pod
uticajem velikog broja porgramskih jezika sa idejom da se izvršava na \emph{Erlang}
...
Poglavlje četiri sadrži opis funkcionalnosti MSNR portala i arhiteturu rešenja. Nakon čega se,
u poglavlju pet prikazuje implementacija serverskog dela upotrebom razvojnog
okruženja \emph{Phoenix}. U poglavlju šest predstavljan je implementacija korisničkog interfejsa
portala korišćenjem \emph{Elm} programskog jezika. Sedmo poglavlje sadrži zaključak celog rada sa
...
strane. Takođe, \emph{Elixir} ne podržava slične operatore koji postoje u Elm-u. Različito ponašanje
...
\begin{minipage}{0.5\textwidth}
  ...
  \end{minted}
\end{minipage}
\captionof{listing}{Upotreba \textbf{pipe} operatora u \emph{Elixir}-u (levo) i Elm-u (desno)}
...
\emph{Phoenix} je okruženja za razvoj svih tipova veb aplikcaija, omogućava HTML renderovanje na serverskoj strani i ima odličnu poršku
za komunikaciju u realnom vremenu. Poseduje i svojstven \emph{LiveView} koncept koji pruža bogato korisničko istustvo u realnom vremenu sa
...
Komanda korišćena za kreiranje projketa:
...
Radno okruženja\emph{Phoenix} dodaje i konfiugracije vezana za okruženje u kom se aplikacija izvršava. 
...
\begin{itemize}
  ...
  \item \emph{MsnrApi.Repo} --- modul za komunikaciju sa bazom
  \item \emph{MsnrApi.Mailer} --- modul za slanje elektronske pošte
\end{itemize}
Kontekst \emph{MsnrApiWeb} sadrži implementaciju za poglede i kontrolore unutar \emph{MVC} arhitekture.
Folder \emph{controllers} je inicijalno prazan, dok su u folderu \emph{views} kreirani pomoćni moduli za prikazivanje grešaka.
Moduli \emph{MsnrApiWeb.Endpoint} i \emph{MsnrApiWeb.Router} imaju ulogu da pripreme HTTP zahtev i proslede ga odgovarajućem
kontroloru.
...
sadžaće skripte vezane za izmenjivanje i popunjavanje baze podataka. Testiranje, kao ni telemetrija, nije tema ovog rada.

\section{Odrada HTTP zahteva}

U osnovi obrade HTTP zahteva nalazi se biblioteka \emph{Plug}, koja se koristi gotovo kroz sve faze obrade zahteva.
Osnovne komponente radnog okruženja \emph{Phoenix}, kao što su \emph{Endpoint}, ruter i kontrolori, su interno utikači (eng. \emph{plug}).
Utičak predstavlja funkciju čija je ulazna i povratna vrednost \emph{Plug.Conn} struktura koja sadrži sve informacije o
primljenom HTTP zahtevu. Veb server daje početne podatke za obradu zahteva, nakon čega \emph{Phoenix} poziva utikače jedan za drugim,
...
\end{listing}
Modul \emph{MsnrApiWeb.Endpoint} predstavlja početnu tačku prilikom obrade HTTP zahteva i sastoji se od niza utikača. Kostur
modula priklazan je u listingu \ref{listing:plugsEndpoint}. Navedeni utikači imaju različite uloge, kao što je prikazivanje statičkih fajlova, logovanje,
parsiranje i drugih karatkerističnih za \emph{Phoenix}. Svaki zahtev prolazi kroz sve navedene utikače. Postavljaju se pomoću makroa \texttt{\textbf{plug}}, a njihovo
...
\end{minted}
\caption{Prikaz pozivanja utikaća pomoču operatora \texttt{\textbf{|>}}}
...
Svaki od tih zahteva proći će kroz prethodno definisani tok, nakon čega se na osnovu metoda HTTP zahteva i nastavka
putanje pozvia određena akcija, tj. funkcija iz navedenog kontorlora. Na primer, za zahtev sa metodom
\emph{GET} i putanjom \emph{/api/auth/refresh} poziva se funkcija \emph{refresh} iz kontolera 
\emph{AuthController}. Za kontoler tipova aktivnosti koristi se funkcija \emph{resource} koja
predstavlja skraćenu sintaksu za navođenje ruta. Jednim pozivom se kreira mapiranje metoda za sedam
(\texttt{\textbf{index}}, \texttt{\textbf{new}}, \texttt{\textbf{create}}, \texttt{\textbf{show}}, \texttt{\textbf{edit}},
\texttt{\textbf{update}}, i \texttt{\textbf{delete}}) akcija koje su komplementarne REST akcijama. Poslednji argument označava da
će se izostaviti akcije \texttt{\textbf{new}} i \texttt{\textbf{edit}} koje se koriste prilikom upotrebe HTML šablon. Takođe je moguće
navesti i samo akcije koje će se koristit pomoći atoma \texttt{\textbf{:only}} umesto \texttt{\textbf{:except}}. U listingu \ref{listing:routes}
...
\subsection{Kontrolori i pogledi}
...
Na primeru kontrolera \emph{ActivityTypeController} u listingu \ref{listing:controller} može se videti generalna struktura konrolera
...
\end{minted}
\caption{Primer strukture kontrolera}
...
On nije sastavni deo \emph{Phoenix} okruženja, ali se podrazumevano uključuje i prilikom kreiranja
...
\begin{itemize}
  \item \emph{Ecto.Repo} --- definisanje omotače oko baze podataka preko koji se ostvaruje komunikacija sa bazom, opisju gde se nalaze podaci
  \item \emph{Ecto.Schema} --- definisanje mapiranja eksternih podatak u Elixir strukture, opisuje šta su podaci
  \item \emph{Ecto.Query} --- definisanje upita u Elixir sintaksi, opisuje kako se čitaju podaci
  \item \emph{Ecto.Changeset} --- praćenje i validiranje izmena, opisuje kako se menjaju podaci
\end{itemize}
Za integraciju sa \emph{Ecto} projektom, \emph{Phoenix} uključuje \texttt{\textbf{phoenix{\textunderscore}ecto}} paket
,adapter za PostgreSQL bazu podatak ---\texttt{\textbf{postgrex}} i \texttt{\textbf{ecto{\textunderscore}sql}} paket
...
zato se navodi lista naziva repozitorijuma. Parametri za definisanje komunikacije sa bazom podatak se postavljaju se posebno za svako okruženje.
...
\end{minted}
\caption{Migracija za kreiranje tabele activity{\textunderscore}types}
...
\begin{table}[h!]
  ...
  \end{tabular}
  \caption{\emph{Ecto.Repo} funkcije za rad sa bazom podatak}
...
Prve dve celine se izvršavaju unutar kontrolora \emph{StudentRegistrationController} čije su
...
U poslednjem koraku registrovanja studenata koristi se kontrolor \emph{PasswordController}.
\begin{listing}[h]
\begin{minted}[fontsize=\footnotesize]{elixir}
...
... 
\end{minted}
\caption{Definisanje putanje za regostrovanje studenata}
...
kontorlera što je prikazano u listingu \ref{listing:index-registrations}.
\begin{listing}[h]
  \begin{minted}[fontsize=\footnotesize]{elixir}
...
Podnošenje zahteva se izvršava slanjem \emph{POST} HTTP zahteva za čiju obradu je dovoljan k\^{o}d generisan komandom
...
U listingu \ref{listing:password-controller} prikazan je kontrolor \emph{PasswordController} sa akcijom \texttt{update}
...
\end{minted}
\caption{Definicija kontorlora \emph{MsnrApiWeb.PasswordController}}
...
se prosleđuje token koji se prilikom svakog HTTP zahteva šalje serveru kroz zaglavlje \emph{Authorization}.
...
\item \emph{acceess{\textunderscore}token} --- Sadrži informacije o korisniku i koristi se za pristupanje portalu. Ima kraće vreme trajanja (do pola sata) i korsniku se prosleđuje kroz telo HTTP odgovora.
...
Prijavaljivanje korisnika izvršava se unutar \texttt{login} akcije kontorlora \emph{AuthController} i
...
\emph{AuthController} u listingu \ref{listing:router} u poglavlju 5.3 prilikom opisa obrade HTTP zahteva.
\begin{listing}[h!]
  \begin{minted}[fontsize=\footnotesize]{elixir}
...
da seone mogu koristit dalje u kontrolorima. Definicija utikača \emph{MsnrApiWeb.Plugs.TokenAuthentication} prikazana je u
...
se prijavili. Neki tipovi aktivnosi, poput recenzija, zahtevaju dodatnu obradu, ali osnovni princip dodele je isti za sve i
...
obeležava da je aktivnost ispunjena. Odabir teme se takođe izvršava unutar \emph{GroupController} upravljača akcijom
...
Za prijave studenata za samu aktivnost ukoliko prijava postoji, korisiti se poseban kontroler --- \emph{SignupController}.
...
\begin{figure}[!h]
  \centering
  \includegraphics[width=0.4\textwidth]{msnr-elm-src.png}
...
U korenu projekta nalazi se osnovna \emph{Main.elm} datotka u kojoj se nalazi funkcija \texttt{main} sa definicijom \emph{Elm} aplikacije.
...
Komande (\texttt{Cmd}) služe za izvršavanje akcija izvan \emph{Elm} okruženja (npr. HTTP zahtevi), a supskripcijama
...
slanjem HTTP zahteva za obnovu tokena. Korisnik je podrazumevano neautentifikovan dok se ne dobije
odgovor servera. 
\subsection{Komande}
Slanje HTTP zahetva, generisanje slučajnih brojeva, određivanje trenutnog vremena i slične operacije
...
Kreiranje komandi za slanje HTTP zahteva izvršava se pomoću \emph{elm/http} paketa.
...
svojstva HTTP zahteva. Poljem \texttt{expect} se označava koji tip podataka se očekuje kao odgovor i kojom porukom
...
token. Funkcijom \texttt{Time.every} se periodično, na određeni broj milisekundi uzima trenutno vreme i prisledjuje se
...
% stoga nije potrebno uključivanje dodatnih 
% ------------------------------------------------------------------------------
\chapter{Zaključak}
% ------------------------------------------------------------------------------
U ovom radu predstavljen je razvoj studentskog portala upotrebom funkcijonalinih
porgramskih jezika \emph{Elm} i \emph{Elixir}. Sam portal predstavlja prototip na
...
potpuno novo iskustvo u ondosu \emph{JavaScript}, sa \emph{Elm} arhitekturom
...
refaktorisanja i sigurnost koju \emph{Elm} zajedno sa komilatorom pruža programerima.

Pre upotrebe portala potrebno je ubaciti testove i telemetriju za praćenje rada.
Kao značajnije unapređenje portala, pored dodavanja formi, trebalo bi razomotriti
upotrbu rešenja za čuvanje dokumenata na oblaku. Na klijentskoj strani potrebno je
