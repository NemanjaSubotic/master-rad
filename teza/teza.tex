...
\chapter{Elixir}
Sa zvanične stranice Elixir-a \cite{elixir}:
\begin{displayquote}
Elixir je dinamički tipiziran, funkcionlni programski jezik dizajniran za izgradnju skalabilnih
i održivih aplikacija.

Elixir koristi Erlang virtualnu mašinu, poznatu po podršci za rad sistema sa malim kašnjenjem,
...
Radeći kao \emph{Ruby} programer, autor jezika, Žozé Valim (José Valim) uvideo je probleme
rada \emph{Ruby on Rails} okruženja na višejezgarnim sistemima, kao i prednosti funkcionalnog
programiranja i Erlang virtualne mašine. Stoga se odlučuje da kreira programski jezik Elixir i 
prvu verziju jezika objavljuje 2011. godine. Najveći uticaj na razvoj Elixir imali su programski
jezici Erlang, sa kojim je semantički vrlo sličnan, i Ruby u smislu sintakse. 
...
japanski naučnik i programer Jukihiro Macumoto (Yukihiro "Matz" Matsumoto). Ruby je dizajniran
...
Budući da je Elixir nastao na ideji upotrebe Erlang virtualna mašine, više o Erlang-u biće rečeno
u sledećem poglavlju. Takođe, pored dva navedena programska jezika, uticaj na razvoj Elixir-a 
imali su Cloujer, Haskell i Python.

\section{Erlang}
Erlang nije samo programski jezik, već i razvojna platforma za izgradnju skalabilnih i pouzdanih
sistema koji neprestano pružaju usluge sa malo ili bez prekida rada. Osmišljena je sredinom 
osamdesetih godina proslog veka, od strane švedske telekomunikacione kompanije Erikson (eng.
\emph{Ericsson}). Da bi Erlang mogao da upravlja telekomunikacionim sistemima kompanije, morao
je da bude pouzdan, skalabilan, da ima brz odziv i bude konstanto dostupan, jer telefonska mreža
...
telekomunikacionih uređaja. Zapravo, to je razvojna platforma koja pruža posebnu podršku
tehničkim, nefunkcionalnim zahtevima kao što su: paralelnost, skalabilnost, tolerancija na
greške, distribuiranost i velika dostupnost. U vreme nastajanja Erlanga, programi su se uglavnom
...
sistema i aplikacija poklapaju se nefunkcionalnim zahtevima za koje Erlang pruža podršku, tako
da je u poslednje vreme privukao veću pažnju. Erlang pokreće razne velike sisteme, među kojima
suaplikacije \emph{WhatsApp} i \emph{WeChat}, \emph{Riak} distribuirana baza podataka i 
\emph{Heroku Cloud}.

Kao razvojna platforma, Erlang se sastoji od jezika, virtualne mašine, razvojnog okvira (eng.
\emph{framework}) i alata.
\subsection{Erlang virtualma mašina --- BEAM}
Jezik Erlang predstavlja primaran način pisanja koda koji se izvršava na Erlang virtualnoj
mašini --- BEAM. BEAM je skraćenica za \emph{Bogdan’s Erlang Abstract Machine}, po Bogumilu
...
virtualne mašine. K\^{o}d napisan u Erlang-u se prevodi u binarni k\^{o}d (eng. \emph{bytecode})
...
\begin{figure}[h]
  ...
  \caption{Način izvršavanja Erlang procesa unutar BEAM-a \cite{elixir}}
  \label{fig:beam}
\end{figure}
...
OTP je skraćenica za \emph{Open Telecom Platform}, što je donekle pogrešan naziv, jer nije
toliko povezana sa telekom sistemima i predstavlja razvojni okvir opšte namene koji apstrahuje
tipične zadatke Erlang sistema:
\begin{itemize}
  \item Obrasce konkurentnosti i distribanosti
  \item Detekciju i oporavak od greški u konkurentim sistemima
...
Elixir predstavlja alternativni načina pisanja programa koji se izvršava na Erlang virtualnoj
...
i metaprogramiranje, podrška za lenjo izračunavanje, kao i to da nije čist funkcionalni jezik.  
\section{Uputstvo za instalaciju}
Proces instalacije je vrlo jednostavan, potrebno je pratiti uputstva sa zvanične Elixir
stranice \cite{elixir}. Provera usprešne instalacije može se izvršiti pokretanjem komande
...
vrednosti, atomi i stringovi. Takođe, u osnovne tipove Elixir ubraja liste i torke. Pregled
osnovnih tipova dat je u listingu \ref{listing:tipoviElixir}, gde se može videti i način pisanja
komentara. Elixir podržava samo linijske komentare i označavaju se simbolom \texttt{\textbf{\#}}.
\begin{listing}[!h]
\begin{minted}{elixir}
iex> 1          # integer
iex> 1.0        # float
...
\label{listing:atomi}
\end{listing}
\subsubsection{Buleanske vrednosti kao atomi}
Elixir zapravo nema poseban tip za buleanske vrednosti, već koristi atome \texttt{\textbf{:true}}
...
true
\end{minted}
\caption{Predstavljanje booleanskih vrednosti preko atoma}
\label{listing:boolAtomi}
\end{listing}

Pored booleanskih vrednosti, jos jedna specifična vrednost predstavljena atomom jeste
...
U Elixir-u koriste UTF-8 kodiranje i pružaju mogućnost ugrađivanja izraza --- interpolaciju
...
\end{listing}
\subsubsection{Bitstringovi i sekvence bajtova (eng. \emph{binaries})}
Kao i kod buleanskih vrednosti, Elixir nema poseban tip za stringove, već za njihovu
reprezentaciju koristi sekvencu bajtova (eng. \emph{binary}) i UTF-8 kodiranje.
Zapravo fundamentalni tip za njihovu reprezentaciju jeste \textbf{bitstring}, koji 
predstavlja neprekidni niz bitova u memoriji. Dok \emph{binary} predstavlja bitstring čiji
je broj bitova deljiv sa osam. Bitstringovi se navode kao sekvenca brojeva unutar znakova
...
(\texttt{\textbf{++}}) i oduzimanje (\texttt{\textbf{-{}-}}) listi, kao i funkcije za određivanje
...
list i poklapanju obrazaca. Primeri upotrebe nekih od navedenih funkcija prikazani su u
listingu \ref{listing:elixirList}, a nešto više o modulu \texttt{\textbf{Enum}} i
funkcijama višeg reda biće rečeno kasnije. 
\begin{listing}[!h]
\begin{minted}{elixir}
...
\end{listing}
\subsubsection{Liste karaktera (eng. \emph{Charlists})}
Lista karaktera je lista celih brojeva, gde svaki element predstavlja jedan karakter.
...
se nalaze funkcije za pristupanje i ažuriranje elemenata, kao i funkcija za odrđivanje broja
...
čiji je priv član (tj. ključna reč) obavezno atom, a drugi može biti bilo kog tipa. Takođe,
...
mogu koristit i funkcije iz modula \texttt{Enum}, jer priadaju nabrojivim tipovima.
...
biti buleanska vrednost, već vrednost bilo kod tipa. Takođe, Elixir ima dvostruke operatore
...
\begin{enumerate}
  ...
  mora biti buleanska vrednosti
  \item \textbf{\texttt{\&\&}}, \textbf{\texttt{||}} i \textbf{\texttt{!}} --- oba operanda 
  mogu biti bilo kog tipa
\end{enumerate}
Operatori čiji operandi mogu biti bilo kog tipa zasnivaju se na konceptu istinitosti, gde
...
\texttt{\textbf{\textunderscore}} se može koristit za poklapanje vrednosti koje se neće
dalje koristit. Primeri poklapanja obrazaca listi i torki dati su u listingu
...
poklapanje prikazano u listingu \ref{listing:elixirPoklapanjeMapa}.
\begin{listing}[!h]
\begin{minted}{elixir}
...
Naziv modula mora počinjati velikim slovom i po konvenciji se pišu u kamilja notaciji, dok je 
...
podrazumevanjih vrednosti, kao i preopterećivanje funkcija, što je i prikazano u listingu
...
Prilikom definisanja funkcija može se koristi poklapanje obrazaca za destrukciju argumenata,
...
moguće koristit zagrade. Takođe, prilikom pozivanja anonimnjih funkcija obavezno je korišćenje
...
već isključivo anonimne. Ovaj nedostatak imenovanih funkcija se može prevazići upotrebom
operatora \texttt{\textbf{\&}} (eng. \emph{capture}), koji imenovanu funkciju pretvara u
anonimnu. Operator \texttt{\textbf{\&}} se može koristiti za kraću sintaksu anonimnih funkcija,
...
Elixir-u. Prilikom definisanje, lambda funkcija ima pristup svim promenljivim unutar opsega u
kom se definiše. Ukoliko koristi promenljive van svog opsega, kreiraju se reference na trenutne
vrednost promenljivih i ponovno vezivanje promenljivih ne utiče na izvršavanje funkcije. U
...
može reći da se vrednost sa leve strane prosleđuje kao poslednji argument funkciji sa desne
...
Atributi u modulu se mogu koristit za definisanje konstanti, takođe Elixir pruža atribute
...
je i specifikacija tipova (eng. \emph{typespecs}) koja se može koristit za definisanje
korisničkih tipova, kao i za anotaciju tipova sličnu anotaciji u Elm-u. Više o specifikaciji
tipova i samim atributima može se videti na zvaničnoj stranici.
...
\section{Makroi}
Makroi predstavljaju jednu od najvažnijih karakteristika Elixir-a u poređenju sa Erlang-om.
Omogućavaju metaprogramiranje, tj. pisanje koda koji generiše k\^{o}d, što dovodi do 
značajnog smanjenja količine šablonskog (eng. \emph{boilerplate}) koda, a samim tim i do
vrlo čitljivog i elegantong koda. Upotrebom makroa se tokom kompilacije vrše transformacije
nad kodom, čime se ne utiče na performase izvršavanja. Markoi i metaprogramiranje nisu predmet
...
Veoma bitan marko koji se često povezuje sa direktivama jeste marko \texttt{\textbf{use}},
koji omogućava ubacivanje spoljne funkcionalnosti (koda) u trenutni modul.
...
Makro\texttt{unless} se zapravo može posmatrati kao \texttt{if not}, tako da sve što važi za
\texttt{if}, važi i za \texttt{unless}. Sintaksa upotrene  oba makroa data je u listingu
...
nije \texttt{nil} ili \texttt{false}. Dok za razliku od Elm-a, \texttt{else} grana nije
obavezna i ukoliko se ne navede, a uslov nije istinit, vrednost izrara je \texttt{nil}.
...
što Elixir pruža mogućnost lenjog izračunavanja kao i upotebu \emph{ubrajanja}
...
veliki broj funkcija za filtriranje, sortiranje, transformasanje i mnogim drugim uobičajenim
operacijama za ovaj tip podataka. Prethodno su navedene liste i mape kao nabrojivi tipovi, 
...
\ref{listing:elixirEnum}, zajedno sa listama i mapa. Funkcije iz modula \texttt{Enum}
mogu raditi sa bilo kojim tipom koji implementira \texttt{Enumerable} protokol. Više
o protokolima biće kasnije u tekstu.
\begin{listing}[!ht]
\begin{minted}{elixir}
...
\end{listing}
\subsubsection{Tokovi (eng. \emph{Streams})}
Tokovi predstavljaju posebnu vrstu nabrojivih tipova koji se mogu koristit za kreiranje
lenjih kompozitnih operacija nad nabrojivim podacima. Tokovi su definisani u modulu
...
\end{listing}
\subsubsection{Ubrajanja}
Filtriranje i mapiranje nad nabrojivim podacima predstaljaju čestu pojavu u Elixir programima,
te su uvedena \emph{ubrajanja} kao sintaksička olakšica (eng. \emph{syntactic sugar}), koja
grupiše ovakve operacije u \texttt{\textbf{for}} specijalnu formu. Upotreba ubrajanja 
prikazana je u listingu \ref{listing:elixirComprehensions}, a više o njima može se pronaći na zvaničnoj stranici.
\begin{listing}[!ht]
\begin{minted}{elixir}
for n <- 1..5, do: n * n
[1, 4, 9, 16, 25]
...
[2, 6, 10, 14, 18]
\end{minted}
\caption{Primeri upotrebe ubrajanja}
...
videti više na zvaničnoj dokumentaciji.
...
Primeri nekoliko implementacij protokola iz listinga može se videti u
