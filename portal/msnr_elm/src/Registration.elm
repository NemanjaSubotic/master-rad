...
import Util exposing (progressLine, submitButton)
        ...
        GotRegistrationResult result ->
            let
                ...
                newModel =
                    case result of
                        Ok _ ->
                            { init | queue = setQueueMessage "Uspešno ste podneli prijavu!" }

                        Err _ ->
                            { model | queue = setQueueMessage "Došlo je do neočekivane greške 😞" }
            in
            ( { newModel | status = Just result, processing = False }, Cmd.none )
