  ...
  case msg of 
    ...
    LoadedActivities activitiesResult ->
      case activitiesResult of
        Ok activities -> 
          let
            foldFn = getFragmentsAndCommands model.currentTimeSec model.studentInfo token
            (fragments, cmds, _) = List.foldr foldFn (Array.empty, [], 0) activities
          in
...
init info = Model 0 Array.empty True info
...
viewFragment 
  : StudentInfo -> (StudentsActivity -> Bool) ->  StudentActivityFragment 
  -> Html Msg
...
getFragmentsAndCommands 
  : Int -> StudentInfo -> String -> StudentsActivity 
